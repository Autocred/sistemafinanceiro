...
function patchConfiguracoes() {
  ...
  const regex = /\{biometriaAtiva && <span style=\{\{ fontSize: 12, color: '#22c55e', fontWeight: 600 \}\}>Ativo neste dispositivo.*?[ \S\s]*?<\/div>\s*<\/div>\s*<div/g;

  const pinUI = `{biometriaAtiva && <span style={{ fontSize: 12, color: '#22c55e', fontWeight: 600 }}>Ativo neste dispositivo ✓</span>}
             </div>
          </div>
  ...
  content = content.replace(regex, pinUI);

  fs.writeFileSync('src/components/Configuracoes.tsx', content, 'utf8');
}

patchConfiguracoes();
console.log('Done patching config');
