    ...
    <div style={{ maxWidth: 1000, margin: '0 auto', paddingBottom: 160 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 20 }}>
        {orcamentos.map(orc => {
          ...
          return (
            <div key={orc.id} className="glass hover-lift" style={{ position: 'relative', overflow: 'hidden', padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  <h3 style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    {orc.categoriaNome}
                  </h3>
                  <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Limite: {formatarMoeda(orc.limite)}</div>
                </div>
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={() => handleEditarOrcamento(orc)} className="action-icon-btn" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', cursor: 'pointer', padding: 8, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)' }} title="Editar Orçamento">
                    <Edit2 size={16} />
                  </button>
                  <button onClick={() => excluirMeta(orc.id)} className="action-icon-btn" style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.2)', cursor: 'pointer', padding: 8, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ef4444' }} title="Excluir Orçamento">
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                {/* ANEL DE PROGRESSO (Activity Ring) */}
                <div style={{ position: 'relative', width: 80, height: 80, flexShrink: 0 }}>
                  <svg width="80" height="80" viewBox="0 0 80 80">
                    ...
                    <circle 
                      ...
                      strokeDashoffset={(2 * Math.PI * 34) - ((percentual > 100 ? 100 : percentual) / 100) * (2 * Math.PI * 34)} 
                      strokeLinecap="round"
                      style={{ transition: 'stroke-dashoffset 1.5s cubic-bezier(0.4, 0, 0.2, 1)', filter: `drop-shadow(0 0 6px ${statusColor}80)` }}
                      transform="rotate(-90 40 40)"
                    />
                  </svg>
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                    <span style={{ fontSize: 16, fontWeight: 900, color: 'var(--text-primary)' }}>{percentual}%</span>
                  </div>
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, fontWeight: 700 }}>
                    <span style={{ color: 'var(--text-muted)' }}>Gasto atual</span>
                    <span style={{ color: 'var(--text-primary)' }}>{formatarMoeda(gasto)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, fontWeight: 700 }}>
                    <span style={{ color: 'var(--text-muted)' }}>Disponível</span>
                    <span style={{ color: statusColor }}>{formatarMoeda(Math.max(0, orc.limite - gasto))}</span>
                  </div>
                  <div style={{ background: `${statusColor}15`, padding: '6px 12px', borderRadius: 8, display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 4, width: 'fit-content' }}>
                    <AlertIcon size={14} color={statusColor} />
                    <span style={{ fontSize: 11, fontWeight: 800, color: statusColor, textTransform: 'uppercase' }}>{alertMsg}</span>
                  </div>
                </div>
              </div>

              {aiSuggestion && (
                <div style={{ fontSize: 12, color: aiColor, background: 'var(--bg-primary)', padding: '12px 16px', borderRadius: 12, border: `1px solid ${aiColor}30`, lineHeight: 1.5, marginTop: 4 }}>
                  {aiSuggestion}
                </div>
              )}
            </div>
